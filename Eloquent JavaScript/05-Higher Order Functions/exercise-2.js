...
(function(){
  ...
  /* */
  let ageDifferences = ANCESTRY.map( ( person, index, ancestry ) => {
    if(!byName[person.mother]) return;

    var birthYear = person.born;
    var mothersBirthYear = byName[ person.mother ].born;

    return birthYear - mothersBirthYear;
  } );

  let averageAgeDifference = average( ageDifferences.filter((person) => person) );
